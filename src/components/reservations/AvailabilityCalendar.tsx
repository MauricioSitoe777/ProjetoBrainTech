...
const STATUS_STYLES = {
  livre: 'bg-zinc-800/60 text-white',
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden max-w-sm mx-auto lg:mx-0">
      <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-800">
        <button type="button" onClick={prevMonth} className="p-1.5 text-white hover:text-white rounded-lg hover:bg-zinc-800">‹</button>
        <span className="text-sm font-medium text-white capitalize">{getMonthLabel(year, month)}</span>
        <button type="button" onClick={nextMonth} className="p-1.5 text-white hover:text-white rounded-lg hover:bg-zinc-800">›</button>
      </div>

      <div className="p-3">
        <div className="grid grid-cols-7 gap-1 mb-2">
          {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(d => (
            <div key={d} className="text-center text-[10px] text-white font-medium py-1">{d}</div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-4 pt-3 border-t border-zinc-800">
          {(Object.entries(STATUS_STYLES) as [keyof typeof STATUS_STYLES, string][]).map(([k, _]) => (
            <div key={k} className="flex items-center gap-1.5 text-[10px] text-white capitalize">
