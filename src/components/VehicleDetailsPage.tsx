...
<<<<<<< HEAD
  const [inPurchaseFlow, setInPurchaseFlow] = useState(false);
  const [selectedFlow, setSelectedFlow] = useState<SimulatorFlow | null>(null);
...
<<<<<<< HEAD
  useEffect(() => {
    const read = () => {
      ...
      if (!raw) {
        setInPurchaseFlow(false);
        setSelectedFlow(null);
        return;
      }
      try {
        const p = JSON.parse(raw);
        setSelectedFlow(p.mode ?? null);
        setInPurchaseFlow(Boolean(p.mode));
      } catch {
        setSelectedFlow(null);
        setInPurchaseFlow(false);
      }
    ...
    read();
    const onSel = () => read();
    window.addEventListener('rentcar:vehicle-selected', onSel as EventListener);
    return () => window.removeEventListener('rentcar:vehicle-selected', onSel as EventListener);
  }, []);

  useEffect(() => {
    const r = sessionStorage.getItem('rentcar:last-route') ?? null;
    setLastRoute(r);
  }, []);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-950 text-white">
        <div className="text-center">
          ...
          >
<<<<<<< HEAD
            {inPurchaseFlow && selectedFlow === 'compra' ? 'Voltar aos Processos' : 'Voltar ao Catálogo'}
=======
            Voltar ao Catálogo
>>>>>>> recuperacao
          </button>
...
<<<<<<< HEAD
          <span className="text-[11px] font-black uppercase tracking-[0.15em]">{inPurchaseFlow && selectedFlow === 'compra' ? 'Voltar aos Processos' : 'Voltar ao Catálogo'}</span>
=======
...
<<<<<<< HEAD
            {(() => {
              const sold = isSoldVehicle(vehicle.id, reservations);
              const last = lastRoute ?? '';
              const cameFromCatalog = last === '' || last === '/' || last.includes('/catalog') || last.includes('catalogo') || last.includes('catalog');
              const cameFromHistory = last.includes('compra') || last.includes('historico') || last.includes('reservas') || last.includes('reservations');
              const showSoldBanner = sold && !inPurchaseFlow && (cameFromHistory || cameFromCatalog);

              if (showSoldBanner) {
                return (
                  <div className="mt-auto rounded-3xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center shadow-xl shadow-emerald-500/10">
                    <p className="text-sm font-black uppercase tracking-[0.2em] text-emerald-300">Viatura Vendida</p>
                    <h2 className="mt-3 text-2xl font-black text-white">Vendida com sucesso</h2>
                    <p className="mt-2 text-sm leading-6 text-zinc-300">
                      Esta viatura já foi registada como vendida no sistema e não está disponível para compra.
                    </p>
                  </div>
                );
              }
