...
export default function Simulator({
  ...
  const suggestions = useMemo(() => {
    ...
    if (!query) return [];
    return allUsers.filter(u => u.nome.toLowerCase().includes(query));
  }, [clientName, allUsers]);
  ...
  useEffect(() => {
    try {
      ...
      if (parsed.id) setSelectedVehicleId(parsed.id);
      if (lockedFlow) {
        setFlow(lockedFlow);
    ...
    if (category === "func_privado") {
      ...
      if (pctEntry > 50) {
        return {
          ...
          why: `Para funcionários privados, a entrada não pode ultrapassar 50% do valor do carro (${fmt(Math.round(vehiclePrice * 0.5))} MT). Estás a colocar mais do que isso.`,
  ...
  const STEP_LABELS = ["Identificação", "Detalhes", "Resumo"] as const;

  return (
    <>
      <section
        id="simulador"
        className="relative bg-zinc-950 flex flex-col overflow-hidden"
      ...
      >
        ...
        {/* ── Step bar ── */}
        <div className="relative z-10 shrink-0 px-5 pt-3.5 pb-3 border-b border-zinc-800/60 flex items-center justify-between gap-4">
          <div className="flex flex-col">
            <span className="text-amber-500 text-[10px] font-bold uppercase tracking-[0.2em] leading-none">
              {lockedFlow === "aluguer" ? "Aluguer" : "Compra & Aluguer"}
            </span>
            <h2 className="text-white text-base font-black tracking-tight leading-tight mt-0.5">Simulador</h2>
          </div>
            ...
            {STEP_LABELS.map((label, i) => (
              <div key={label} className="flex items-center gap-1.5">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold transition-all duration-200 ${
                    i === step
                      ? "bg-amber-500 text-zinc-950"
                      : i < step
                      ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 cursor-pointer"
                      : "bg-zinc-800/80 text-zinc-500 border border-zinc-700/60 cursor-default"
                  }`}
                >
                ...
                {i < 2 && (
                  <div className={`w-3 h-px shrink-0 transition-colors duration-300 ${i < step ? "bg-emerald-500/50" : "bg-zinc-700"}`} />
                )}
              </div>
            ))}
          </div>
          ...
          {showClose && (
            <button
              ...
              onClick={() => {
                try { window.dispatchEvent(new CustomEvent("rentcar:close-simulator")); } catch { /* ignore */ }
              }}
              className="shrink-0 w-8 h-8 rounded-full bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-white transition flex items-center justify-center text-lg font-bold"
            >
              ×
            </button>
          )}
        ...
        <div className="flex-1 overflow-hidden relative z-10">
          ...
          >
            ...
            <div className="w-full shrink-0 h-full overflow-y-auto">
              <div className="max-w-lg mx-auto px-5 py-5 flex flex-col h-full">
                <div className="flex-1 flex flex-col gap-5">

                  <div>
                    <p className="text-zinc-500 text-[11px] font-semibold uppercase tracking-widest mb-3">1 — Tipo de serviço</p>
                    {!lockedFlow ? (
                      <div className="grid grid-cols-2 gap-2">
                        {([
                          { key: "compra",  icon: <IconCar size={14} />,  label: "Comprar",  sub: "Prestações mensais" },
                          { key: "aluguer", icon: <IconKey size={14} />,  label: "Alugar",   sub: "Diário ou mensal" },
                        ] as const).map((o) => (
                          <button
                            key={o.key}
                            onClick={() => setFlow(o.key)}
                            className={`py-4 px-4 rounded-xl text-left transition-all duration-200 border ${flow === o.key
                          ...
                          >
                            <p className={`text-sm font-black flex items-center gap-1.5 ${flow === o.key ? "text-zinc-950" : "text-white"}`}>
                              {o.icon}{o.label}
                            </p>
                      ...
                      <div className="py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-1.5 bg-amber-500 text-zinc-950">
                        {lockedFlow === "aluguer" ? <><IconKey size={14} /> Aluguer</> : <><IconCar size={14} /> Compra</>}
                      </div>
                    )}
                  </div>

                  <div>
                    <p className="text-zinc-500 text-[11px] font-semibold uppercase tracking-widest mb-3">2 — Identificação do cliente</p>
                    <div className="flex flex-col gap-3">
                      <div className="relative">
                        <label className="text-zinc-400 text-[11px] font-semibold block mb-1.5 tracking-wide">Nome completo</label>
                        <input
                          value={clientName}
                          onChange={(e) => { if (authUser) return; setClientName(e.target.value); setShowSuggestions(true); }}
                          onFocus={() => !authUser && setShowSuggestions(true)}
                          onBlur={() => setTimeout(() => setShowSuggestions(false), 200)}
                          placeholder="Ex: Ana Mussa"
                          readOnly={!!authUser}
                          className={`w-full rounded-lg bg-zinc-900 border px-3 py-2.5 text-sm text-white font-semibold placeholder:text-zinc-600 outline-none transition-all ${
                            authUser
                        ...
                        {showSuggestions && suggestions.length > 0 && !authUser && (
                          <div className="absolute left-0 right-0 mt-1.5 bg-zinc-900 border border-zinc-700/80 rounded-xl max-h-48 overflow-y-auto z-20 shadow-[0_8px_32px_-4px_rgba(0,0,0,0.7)] divide-y divide-zinc-800/80">
                            {suggestions.map(u => (
                      ...
                      <div>
                        ...
                        <div className={`flex items-center w-full rounded-lg bg-zinc-900 border overflow-hidden transition-all ${
                          authUser
                          ...
                          <div className="pl-3 pr-2.5 py-2.5 text-[11px] text-amber-500/80 font-semibold bg-zinc-800/60 border-r border-zinc-800 shrink-0">+258</div>
                          <input
                            value={clientContact}
                            onChange={(e) => !authUser && setClientContact(formatContact(e.target.value))}
                            placeholder="84 000 0000"
                            readOnly={!!authUser}
                            className={`w-full bg-transparent px-3 py-2.5 text-sm text-white font-semibold placeholder:text-zinc-600 outline-none tabular-nums ${authUser ? "cursor-not-allowed" : ""}`}
                          />
                        </div>
                      </div>
                    </div>
                ...
                <div className="pt-4 pb-3 shrink-0">
                  <button
                    onClick={() => setStep(1)}
                    className="w-full py-3 rounded-xl font-bold text-sm bg-amber-500 text-zinc-950 hover:bg-amber-400 shadow-md shadow-amber-500/20 hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 flex items-center justify-center gap-2"
                  >
                    Seguinte
            ...
            <div className="w-full shrink-0 h-full overflow-y-auto">
              <div className="max-w-lg mx-auto px-5 py-5 flex flex-col gap-4">
                ...
                {flow === "compra" ? (
                  <>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <label className="text-zinc-400 text-[11px] font-semibold tracking-wide uppercase">Tipo de Funcionário</label>
                        {currentUser?.category && (
                          <span className="text-[10px] font-semibold text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 px-2 py-0.5 rounded-full">Perfil</span>
                        )}
                      </div>
                      {(() => {
                        const PROFILES = [
                        ...
                        return (
                          <>
                              ...
                              {PROFILES.map(p => {
                                ...
                                return (
                                  <button key={p.key} type="button" disabled={locked}
                                    ...
                                    className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border text-[11px] font-semibold transition-all ${
                            ...
                            {active && <p className="text-[11px] text-zinc-500 mt-1.5">{active.rules}</p>}
                          </>
                        );
                      })()}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <NumberField label="💰 Preço do Veículo" value={vehiclePrice}
                        onChange={(v) => setVehiclePrice(Math.min(8_000_000, Math.max(0, v)))} min={0} suffix="MT" disabled={!isAdmin} />
                    ...
                    {/* Modo de pagamento */}
                    <div>
                      <label className="text-zinc-400 text-[11px] font-semibold block mb-2 tracking-wide uppercase">Como vai pagar?</label>
                      <div className="grid grid-cols-2 gap-2">
                        {([
                          ...
                          <button key={o.key} onClick={() => setPaymentPlan(o.key)}
                            className={`py-3.5 px-4 rounded-xl text-left transition-all border ${paymentPlan === o.key
                              ? "bg-amber-500 border-amber-400 text-zinc-950 shadow-lg shadow-amber-500/20"
                              : "bg-zinc-800/60 border-zinc-700/80 text-white hover:bg-zinc-800 hover:border-zinc-600"}`}
                          >
                    ...
                    {paymentPlan === "prestacoes" && (
                      <>
                        <div className={`rounded-xl px-3.5 py-3 border text-[11px] leading-relaxed ${
                          ...
                          {category === "func_publico" && <span>✅ <b>Funcionário Público:</b> não precisa de dar entrada.</span>}
                          {category === "func_privado" && <span>ℹ️ <b>Funcionário Privado:</b> entrada entre <b>10%</b> ({fmt(vehiclePrice * 0.1)} MT) e <b>50%</b> ({fmt(vehiclePrice * 0.5)} MT).</span>}
                          {category === "empreendedor" && <span>ℹ️ <b>Empresário:</b> entrada mínima de <b>75%</b> = <b>{fmt(vehiclePrice * 0.75)} MT</b>.</span>}
                        </div>
                            ...
                            <div className="flex gap-1.5 mt-1.5 flex-wrap">
                              {category === "func_publico" && (
                                <button type="button" onClick={() => setDownPayment(0)}
                                  className="text-[9px] font-bold px-2 py-0.5 rounded bg-zinc-800 text-white border border-zinc-700">Sem entrada</button>
                              )}
                              ...
                              {category === "empreendedor" && (
                                <button type="button" onClick={() => setDownPayment(Math.round(vehiclePrice * 0.75))}
                                  className="text-[9px] font-bold px-2 py-0.5 rounded bg-amber-500/15 text-amber-400 hover:bg-amber-500/25 border border-amber-500/30 transition-all">Mín 75%</button>
                              )}
                            </div>
                          </div>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      <div className="bg-zinc-800/40 rounded-xl border border-zinc-700/60 p-2.5">
                        ...
                        <input type="date" value={dataInicio} onChange={(e) => setDataInicio(e.target.value)}
                          className="w-full rounded-lg bg-zinc-900 border border-zinc-700 px-3 py-2 text-xs text-white font-bold outline-none focus:border-amber-500 mb-1.5" />
                        <input type="time" value={horaLevantamento} onChange={(e) => setHoraLevantamento(e.target.value)}
                          className="w-full rounded-lg bg-zinc-900 border border-zinc-700 px-3 py-2 text-xs text-white font-bold outline-none focus:border-amber-500" />
                      </div>
                      <div className="bg-zinc-800/40 rounded-xl border border-zinc-700/60 p-2.5">
                        <label className="text-zinc-400 text-[10px] font-bold block mb-1 uppercase tracking-wider">Devolução</label>
                        <input type="date" value={dataFim} onChange={(e) => setDataFim(e.target.value)}
                          className="w-full rounded-lg bg-zinc-900 border border-zinc-700 px-3 py-2 text-xs text-white font-bold outline-none focus:border-amber-500 mb-1.5" />
                        <input type="time" value={horaDevolucao} onChange={(e) => setHoraDevolucao(e.target.value)}
                          className="w-full rounded-lg bg-zinc-900 border border-zinc-700 px-3 py-2 text-xs text-white font-bold outline-none focus:border-amber-500" />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        value={motivoViagem} onChange={(e) => setMotivoViagem(e.target.value)}
                        placeholder="Ex: Viagem de negócios à Beira, férias em Bilene..." rows={2}
                        className="w-full rounded-lg bg-zinc-900 border border-zinc-700/80 px-3 py-2 text-sm text-white font-medium placeholder:text-zinc-600 outline-none focus:border-amber-500/60 focus:ring-2 focus:ring-amber-500/10 resize-none transition-all"
                      />
                    </div>

                    <div className="rounded-xl border border-zinc-700/60 bg-zinc-800/30 px-4 py-3 flex items-center gap-3">
                      <div className="shrink-0 w-8 h-8 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
                        </svg>
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider mb-0.5">Local fixo</p>
                        <p className="text-xs font-semibold text-white truncate">Escritório Central · Av. Julius Nyerere, Maputo</p>
                      </div>
                      <span className="shrink-0 text-[10px] bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-full px-2 py-0.5 font-semibold">Fixo</span>
                    </div>
                    ...
                    {(() => {
                      ...
                      return (
                        <div className={`rounded-xl border transition-all duration-200 ${comMotorista ? "border-amber-400/30 bg-amber-400/5" : "border-zinc-700/60 bg-zinc-800/30"}`}>
                          <button type="button" onClick={() => { setComMotorista(v => !v); setMotoristaId(""); }}
                            className="w-full flex items-center justify-between px-4 py-3">
                            <div className="flex items-center gap-3">
                              <span className="text-base">🧑‍✈️</span>
                              <div className="text-left">
                                <p className={`text-sm font-semibold ${comMotorista ? "text-amber-400" : "text-white"}`}>Com motorista</p>
                                <p className="text-[11px] text-zinc-500">Condutor profissional incluído</p>
                              </div>
                            </div>
                          ...
                          {comMotorista && (
                            <div className="px-4 pb-3">
                              {disponiveis.length === 0 ? (
                                <p className="text-[11px] text-zinc-500 italic">Sem motoristas disponíveis.</p>
                              ) : (
                                <select value={motoristaId} onChange={e => setMotoristaId(e.target.value)}
                                  className="w-full bg-zinc-900 border border-zinc-700/80 text-white rounded-lg px-3 py-2 text-sm font-medium focus:border-amber-400/60 outline-none transition-all">
                    ...
                    {dateValidation && !dateValidation.valid && (
                      <div className="flex items-start gap-2.5 text-[11px] text-red-300 bg-red-500/10 border border-red-500/25 rounded-xl px-3.5 py-2.5 font-medium">
                        <span className="shrink-0">⚠️</span><span>{dateValidation.errors[0]}</span>
                      </div>
                    )}
                    {availability && !availability.available && dateValidation?.valid && (
                      <div className="flex items-start gap-2.5 text-[11px] text-red-300 bg-red-500/10 border border-red-500/25 rounded-xl px-3.5 py-2.5 font-medium">
                        <span className="shrink-0">🔒</span><span>{availability.conflicts[0]}</span>
                      </div>
                    )}

                    <div className={`grid gap-3 ${isAdmin ? "grid-cols-3" : "grid-cols-2"}`}>
                      <NumberField label="Dias" value={days} onChange={(v) => setDays(Math.max(1, Math.round(v)))} min={1} step={1} suffix="dias" disabled={dateValidation?.valid === true} />
                      <NumberField label="Custo diário" value={dailyRate} onChange={(v) => setDailyRate(Math.max(0, v))} min={0} step={100} suffix="MT/dia" disabled={!isAdmin} />
                      {isAdmin && (
                        <div className="relative">
                          <NumberField label="Desconto" value={discountPct} onChange={(v) => setDiscountPct(Math.min(100, Math.max(0, v)))} min={0} step={1} suffix="%" />
                          {days >= 7 && (
                    ...
                    {isAdmin && (
                      <div className="grid grid-cols-2 gap-3">
                        <NumberField label="Taxa de limpeza" value={cleaningFee} onChange={(v) => setCleaningFee(Math.max(0, v))} min={0} step={50} suffix="MT" />
                        <NumberField label="Taxa de logística" value={logisticsFee} onChange={(v) => setLogisticsFee(Math.max(0, v))} min={0} step={100} suffix="MT" />
                        <NumberField label="Outras taxas" value={otherFees} onChange={(v) => setOtherFees(Math.max(0, v))} min={0} step={100} suffix="MT" />
                        <NumberField label="Caução" value={deposit} onChange={(v) => setDeposit(Math.max(0, v))} min={0} step={500} suffix="MT" />
                      </div>
                    )}
                  </>
                )}
                ...
                {/* Step 1 footer */}
                <div className="flex gap-2 pt-2 pb-3 shrink-0">
                  <button onClick={() => setStep(0)}
                    className="px-4 py-3 rounded-xl font-bold text-xs border border-zinc-700/80 bg-zinc-800/60 text-zinc-300 hover:bg-zinc-800 hover:text-white transition-all flex items-center gap-1.5">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
                    Voltar
                  </button>
                  <button onClick={() => setStep(2)}
                    className="flex-1 py-3 rounded-xl font-bold text-sm bg-amber-500 text-zinc-950 hover:bg-amber-400 shadow-md shadow-amber-500/20 hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 flex items-center justify-center gap-2">
                    Ver Resumo
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                  </button>
                </div>
              ...
              <div className="max-w-lg mx-auto px-5 py-5 flex flex-col gap-4">
                ...
                {flow === "aluguer" && (
                  <div className="flex items-center gap-1">
                    {[
                      { label: "Datas",      done: !!(dataInicio && dataFim),                              error: false },
                      { label: "Válidas",    done: dateValidation?.valid === true,                         error: dateValidation?.valid === false },
                      { label: "Disponível", done: availability?.available === true,                       error: availability?.available === false },
                      { label: "Pronto",     done: rentalDetailsOk && clientName.trim().length >= 2,       error: false },
                    ].map((s, i, arr) => (
                      <div key={s.label} className="flex items-center gap-1 flex-1">
                        <div className="flex flex-col items-center flex-1">
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-black transition-all duration-300 ${
                            s.error  ? "bg-red-500/20 border border-red-500/50 text-red-400"
                            : s.done ? "bg-emerald-500/20 border border-emerald-500/50 text-emerald-400"
                            : "bg-zinc-800 border border-zinc-700 text-zinc-500"
                          }`}>{s.error ? "✕" : s.done ? "✓" : i + 1}</div>
                          <span className={`text-[9px] font-bold mt-0.5 ${s.error ? "text-red-400" : s.done ? "text-emerald-400" : "text-zinc-500"}`}>{s.label}</span>
                        </div>
                        {i < arr.length - 1 && (
                          <div className={`h-px flex-1 mb-4 transition-colors duration-300 ${s.done && !s.error ? "bg-emerald-500/40" : "bg-zinc-700"}`} />
                        )}
                      </div>
                    ))}
                  </div>
                ...
                {/* Hero total */}
                <div className="bg-zinc-900/80 rounded-2xl border border-amber-500/25 p-5 relative overflow-hidden shadow-[0_0_60px_-8px_rgba(216,160,32,0.12),0_8px_40px_-8px_rgba(0,0,0,0.6)]">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/8 blur-3xl -mr-8 -mt-8 pointer-events-none" />
                  <div className="relative z-10">
                    <div className="text-zinc-500 text-[11px] mb-1.5 font-semibold uppercase tracking-[0.18em]">
                      {flow === "compra"
                        ? paymentPlan === "prestacoes" ? "Prestação Mensal" : "Total (Pronto Pagamento)"
                        : "Total a pagar"}
                    </div>
                    <div className="flex items-baseline gap-2 flex-wrap mb-2">
                      <span className="text-4xl font-black text-white tracking-tighter tabular-nums">
                        {flow === "compra"
                          ? paymentPlan === "prestacoes" ? fmt(purchasePMT) : fmt(vehiclePrice)
                          : fmt(rentalTotal)}
                        <span className="text-xl text-amber-500 ml-1.5 font-black">MT</span>
                      </span>
                    </div>
                ...
                {/* Elegibilidade (compra prestações) */}
                {flow === "compra" && paymentPlan === "prestacoes" && (
                  <div className={`rounded-xl border overflow-hidden ${eligivel ? "border-emerald-500/20" : "border-red-500/20"}`}>
                    <div className={`flex items-center gap-2.5 px-4 py-2.5 ${eligivel ? "bg-emerald-500/10" : "bg-red-500/8"}`}>
                      {eligivel ? (
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2.5"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                      ) : (
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#f87171" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
                      )}
                    ...
                    <div className="px-4 py-3 space-y-2 bg-zinc-900/50">
                      <p className={`text-xs font-semibold ${eligivel ? "text-emerald-300" : "text-red-300"}`}>{financingStatus.title}</p>
                      {financingStatus.why && (
                      ...
                      {!eligivel && financingStatus.fix && (
                        <div className="bg-amber-500/8 rounded-lg px-3 py-2 border border-amber-500/20">
                ...
                {flow === "compra" ? (
                  ...
                  <div className="bg-zinc-800/25 rounded-xl border border-zinc-700/40 px-4 py-3.5 space-y-2">
                    {([
                    ...
                    <div className="flex items-center justify-between gap-2 pt-2 mt-0.5 border-t border-zinc-700/50">
                ...
                {/* Errors / blocked */}
                {submitError && (
                  <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/25 text-red-300 text-[11px] font-medium">
                    <span>⚠️</span><span>{submitError}</span>
                  </div>
                )}
                {isBlocked && (
                  <div className="rounded-xl px-4 py-3.5 bg-red-500/10 border border-red-500/25">
                    ...
                    <p className="text-zinc-400 text-[11px] leading-relaxed">
                      {isRestricted
                        ...
                        : "Operação bloqueada devido a pendências financeiras. Contacte a administração."}
                    </p>
                  </div>
                )}
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl font-bold text-sm uppercase tracking-widest transition-all duration-200 ${
                      (!authUser ? clientName.trim().length >= 2 : canSubmit)
                        ? "bg-amber-500 text-zinc-950 hover:bg-amber-400 shadow-lg shadow-amber-500/20 hover:scale-[1.01] active:scale-[0.99]"
